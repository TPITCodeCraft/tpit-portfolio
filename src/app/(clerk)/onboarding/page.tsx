	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		try {
			...
			if (res?.message) {
				...
				// Check if metadata is actually updated
				const checkResponse = await fetch("/api/onboarding/check");
				const checkData = await checkResponse.json() as { onboardingComplete: boolean; metadata: any };
				
				if (checkData.onboardingComplete) {
					// Force navigation to home page with cache busting
